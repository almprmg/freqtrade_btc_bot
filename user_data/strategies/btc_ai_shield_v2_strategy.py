"""BtcAiShieldV2Strategy — AI Shield with SIGMOID halving sizing.

V1 used linear: target = base * (0.5 + cycle_bias / 2)
This means cycle_bias=+1 -> full size, 0 -> half, -1 -> zero. Symmetric.

V2 uses a non-linear sigmoid + per-phase tweaks:
  - ACCUMULATION (0-180d post-halving): BOOST. We want max exposure
    when the cycle is starting (highest expected forward return).
  - EARLY_BULL (180-365d): full size.
  - PARABOLIC (365-540d): TAPER. Late-cycle euphoria is risky.
  - DISTRIBUTION (540-700d): quick exit.
  - BEAR (700-900d): force 0.
  - REACCUMULATION (900+): gradual rebuild.

We achieve this by remapping cycle_bias through a sigmoid + phase
shift before computing target. The result is a curve that's STEEP
during transitions (post-halving entry, pre-distribution exit) and
FLAT during plateaus (mid-bull, mid-bear).

Formula:
  sigmoid(x, k=4, c=0) = 1 / (1 + exp(-k*(x-c)))
  shift_per_phase:
    ACCUMULATION   +0.10  (boost: makes sigmoid output higher)
    EARLY_BULL     +0.05
    PARABOLIC      -0.10  (taper: makes sigmoid output lower)
    DISTRIBUTION   -0.30  (strong taper)
    BEAR           -0.50  (force low)
    REACCUMULATION -0.10
  target = base_target * sigmoid(cycle_bias + shift, k=4)

Combined with Shield + Anomaly:
  if Shield BEAR or anomaly: target = 0
  else: target = base_target * sigmoid(adjusted_bias, k=4)
"""
...
class BtcAiShieldV2Strategy(IStrategy):
    ...
    def populate_entry_trend(self, dataframe: pd.DataFrame, metadata: dict) -> pd.DataFrame:
        ...
        df.loc[enter, "enter_tag"] = "ai_shield_v2:sigmoid"
        return df
    ...
    def populate_exit_trend(self, dataframe: pd.DataFrame, metadata: dict) -> pd.DataFrame:
        ...
        df.loc[exit_cond, "exit_tag"] = "ai_shield_v2:bearish"
        return df
