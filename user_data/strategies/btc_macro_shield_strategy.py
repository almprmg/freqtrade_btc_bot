"""BtcMacroShieldStrategy — Calendar Shield + Macro Economic Signals.

Extends BtcCalendarShieldStrategy with US macro economic indicators:
  - DXY (Dollar Index)  — inverse correlation with BTC
  - VIX                 — fear gauge (high VIX = bearish crypto)
  - SPY (S&P 500)       — broad risk asset correlation
  - QQQ (NASDAQ)        — tech-heavy, strong BTC correlation
  - TNX (10Y yield)     — rates pressure

Composite signal: `macro_risk_on` ∈ [-1, +1]
  - Empirically correlated +0.29 with BTC 30d forward returns (n=2214)
  - Quintile Q1 (risk-off): BTC -7.9% / 30d, 33% win rate
  - Quintile Q4 (risk-on):  BTC +13.4% / 30d, 68% win rate

Integration:
  adjusted_bias = cycle_bias + phase_shift + calendar_tilt + MACRO_WEIGHT × risk_on
  Where MACRO_WEIGHT = 0.25 (calibrated to match calendar tilt magnitude)

Plus hard rule:
  if macro_risk_on < -0.5 AND already_in_position: exit (extreme risk-off)
"""
...
import os
COIN = os.environ.get("MACRO_COIN", "BTC")

N_CONFIRM = 3
BASE = 0.85
SIGMOID_K = 4.0
MACRO_WEIGHT = float(os.environ.get("MACRO_W", "0.10"))  # default 0.10 (was 0.25 too high)
MACRO_RISK_OFF_EXIT = -0.5     # extreme risk-off forces exit
...
class BtcMacroShieldStrategy(IStrategy):
    ...
    def populate_indicators(self, dataframe: pd.DataFrame, metadata: dict) -> pd.DataFrame:
        ...
        df["calendar_tilt"] = cal_tilt.values

        # ===== MACRO RISK-ON SIGNAL =====
        df["macro_risk_on"] = 0.0
        if not _MACRO.empty and "macro_risk_on" in _MACRO.columns:
            d = pd.to_datetime(df["date"], utc=True).dt.normalize()
            # ffill — macro data may be on business days only
            macro_aligned = _MACRO["macro_risk_on"].ffill()
            df["macro_risk_on"] = d.map(macro_aligned).ffill().fillna(0.0)

        # Macro tilt (clipped to ±0.30 max effect)
        df["macro_tilt"] = (MACRO_WEIGHT * df["macro_risk_on"]).clip(-0.30, 0.30)

        # Sigmoid sizing — cycle + phase + calendar + macro
        shifts = df["cycle_phase"].map(PHASE_SHIFTS).fillna(0.0).astype(float)
        adjusted_bias = (
            df["cycle_bias"].astype(float)
            + shifts
            + df["calendar_tilt"]
            + df["macro_tilt"]
        )
        ...
        # Hard macro risk-off rule
        df.loc[df["macro_risk_on"] < MACRO_RISK_OFF_EXIT, "ai_target"] = 0.0
    ...
    def populate_entry_trend(self, dataframe: pd.DataFrame, metadata: dict) -> pd.DataFrame:
        ...
        df.loc[enter, "enter_tag"] = "macro_shield:enter"
        return df
    ...
    def populate_exit_trend(self, dataframe: pd.DataFrame, metadata: dict) -> pd.DataFrame:
        ...
        exit_cond = (
            ...
            | (df["anomaly"] == 1)
            | (df["ai_target"] < 0.20)
            | (df["macro_risk_on"] < MACRO_RISK_OFF_EXIT)
        )
        df.loc[exit_cond, "exit_long"] = 1
        df.loc[exit_cond, "exit_tag"] = "macro_shield:exit"
        return df
...
__all__ = ["BtcMacroShieldStrategy"]
