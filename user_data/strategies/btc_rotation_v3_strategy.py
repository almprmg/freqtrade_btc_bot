"""BtcRotationV3Strategy — V2 logic + per-asset DD circuit breaker."""
...
_CANDIDATE_PATHS = [
    Path("/freqtrade/user_data/data/rotation_signal_v3.feather"),
    Path(__file__).resolve().parents[1] / "data" / "rotation_signal_v3.feather",
    Path(__file__).resolve().parents[2] / "user_data" / "data" / "rotation_signal_v3.feather",
]
...
class BtcRotationV3Strategy(IStrategy):
    ...
    def populate_entry_trend(self, dataframe, metadata):
        ...
        df.loc[enter, "enter_tag"] = f"rotv3:{_pair_to_coin(metadata['pair']).lower()}"
        return df

    def populate_exit_trend(self, dataframe, metadata):
        df = dataframe
        df.loc[~df["is_my_turn"], "exit_long"] = 1
        df.loc[~df["is_my_turn"], "exit_tag"] = "rotv3:rotated"
        return df
