"""Compare BtcVolShieldStrategy (4 modes) vs Pure Shield, year-by-year on BTC."""
...
COMBOS = [
    ("VS_CLASSIC",   "BtcVolShieldStrategy",     "VS_MODE", "VS_CLASSIC",   "config.volshield.json"),
    ("VS_LOOSE",     "BtcVolShieldStrategy",     "VS_MODE", "VS_LOOSE",     "config.volshield.json"),
    ("VS_AGGR",      "BtcVolShieldStrategy",     "VS_MODE", "VS_AGGR",      "config.volshield.json"),
    ("VS_TIGHT",     "BtcVolShieldStrategy",     "VS_MODE", "VS_TIGHT",     "config.volshield.json"),
...
def main():
    print(f"Testing {len(COMBOS)} variants x {len(YEARS)} years...")
    rows = []
    ...
    df.to_csv(REPO / "research" / "volshield_results.csv", index=False)
    return 0
